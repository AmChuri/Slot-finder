...
describe('Coding challenge calendar tests', () => {
  test.each(tests)('%s', async (_, test_data) => {
    ...
    expect(response.data.length).toBe(
      test_data.expectedResult.length
    );
    console.log(
      response.data,
    ...
    for (let i = 0; i < test_data.expectedResult.length; i++) {
      expect(response.data[i].available_count).toBe(
        test_data.expectedResult[i].available_count
      );
      expect(response.data[i].start_date).toBe(
        test_data.expectedResult[i].start_date
      );
    }
  });
});
